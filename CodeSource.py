...
TREE_FATHER = list() 			# Premier niveau de notre tree-view de forme (f'{ip_source}', f'{ip_destination}', f'{P_ACTU}', f'{L_TAILLE[T_ACTU].get("tot")}')
TREE_CHILDREN = list() 			# Deuxieme niveau de notre tree-view ({"ETHERNET":"","IP":"","UDP":"","DHCP":"", "DNS":""})
RES = list() 					# liste de dict tel que pour chaque RES[i] on a {ETHERNET,IP,UDP,DHCP,UDP} en clés avec pour valeur le message receuilli dans la couche
T_ACTU = 0 						# n° de la trame actuellement en analyse afin de faciliter l'ajout dans TREE_FATHER ET TREE_CHILDREN ( la première étant à 0 )
...
def lecture_fichier(file):
	""" lecture du fichier 'file' si celà est possible """
	if file != '' :
		file = open(file, "r")
		r = file.readlines()
		if len(r) == 0:
			print("Le fichier est soit inaccessible, inexistant ou vide")
			exit()
		else :
			return r
...
def det_compressed(lignes, i, nbcol2, nboct_lu, name_p):
	""" Fonction recursive permettant le decodage des noms compresses de DNS """
	i_st = i
	nbcol2_st = nbcol2
	...
	name_msg = ""
	if int(L[3][0],16) > 11:
		c_bin_util = hex(int(bin(int(L[3],16))[4:],2))
		...
		name_pointeur = int(c_bin_util + L[3],16)
...
def bin_sansb(bin):
	""" Recupere la chaine de caractere du binaire passe en parametre sans le prefixe 0b """
	s = ""
	if len(bin) == 0:
		print("erreur")
		exit()
...
def lecture_Ethernet(lignes, li, nboct_lu):
	""" Renvoie l'analyse de toute la trame en appelant lecture_IP, la taille totale de la trace
		ainsi que le n° de lignes à traiter dans la suite """
	i = li
...
def lecture_IP(lignes, li, nbcol, nboct_l):
	...
	if proto == 17:
		...
		Message += "\nAttention! le trame présente une erreur sur la ligne: ", i+1, " UDP ne peut avoir que 67 OU 53 comme valeur pour le port source ou destination\n"
		return [nboct_lu,Message,TOT_LEN]
...
def etude_UDP(lignes,i,nbcol2,nboct_lu):
	...
	M_tmp = ""
	if (proto_udp1 == 67 or proto_udp2 == 67) or (proto_udp1 == 68 or proto_udp2 == 68):
		P_ACTU = "DHCP"
...
def etude_DHCP(lignes,i,nbcol2,nboct_lu):
	...
	L = etude_option_DHCP(lignes,i,nbcol2,nboct_lu,a_lire,option_len)
	Message += L[0]
	prot = L[1]
...
def etude_option_DHCP(lignes,i,nbcol2,nboct_lu,a_lire,option_len):
...
def etude_DNS(lignes,i,nbcol2,nboct_lu):
	""" Permet l'analyse de l'applicaiton DNS """
	...
	if int(nb_anws) > 0 :
		...
		for j in range(0, int(nb_anws)):
			...
			c_bin_util = (bin(int(L[3][0],16)))[4:]
			stock = hex(int(c_bin_util,2)) + L[3][1:]
			name_pointeur = int(stock,16)
